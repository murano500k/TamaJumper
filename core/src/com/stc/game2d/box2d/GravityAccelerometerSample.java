...
import static com.stc.game2d.Config.GRAVITY_MULTIPLIER;
import static com.stc.game2d.Config.MAX_SPAWNED_BALLS;
...
public class GravityAccelerometerSample extends Sample {
	...
	private void processAccelerometer() {
		...
		float y = Gdx.input.getAccelerometerY()*GRAVITY_MULTIPLIER;
		float x = Gdx.input.getAccelerometerX()*GRAVITY_MULTIPLIER;
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (spawnedBalls < MAX_SPAWNED_BALLS) {
			...
			FixtureDef fixtureDef = Box2DFactory.createFixture(shape, 2.5f,
					0.25f, 0.75f, false);
			Box2DFactory.createBody(world, BodyDef.BodyType.DynamicBody, fixtureDef,
